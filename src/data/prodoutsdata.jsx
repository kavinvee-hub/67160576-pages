...
let prodoutsWithPrice = [];

export function fetchProdouts() {
    if (prodoutsWithPrice.length === 0) {
        prodoutsWithPrice = prodouts.map((prodout) => {
            let updatedProdout = { ...prodout };
            delete updatedProdout.albumId;
            delete updatedProdout.url;
            updatedProdout.price = Math.round(Math.random() * 9000) / 100 + 10;

            return updatedProdout;
        });
    }

    return prodoutsWithPrice;
}
